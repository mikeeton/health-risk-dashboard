...
import DataProvenanceBadge, { isSyntheticSource } from "../components/DataProvenanceBadge";
...
export default function Dashboard() {
  ...
  const containsSyntheticData = patientData.some((record) => isSyntheticSource(record.source));
    ...
    <>
      <div ref={reportRef} className="dashboard-shell space-y-8">
        {containsSyntheticData && (
          <aside className="flex flex-wrap items-center gap-3 rounded-2xl border border-violet-300 bg-violet-50 px-5 py-4 text-sm text-violet-950 dark:border-violet-800 dark:bg-violet-950/30 dark:text-violet-100">
            <DataProvenanceBadge source="simulator" />
            <span>Some displayed readings were generated for demonstration and must not be interpreted as real patient measurements.</span>
          </aside>
        )}
        ...
        <section id="stats" className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-5 scroll-mt-28">
          <StatCard
            ...
            description="Calculated indicator from stored risk scores"
            status={
              stats.avgRisk >= 6
                ? "alert"
                : stats.avgRisk >= 4
